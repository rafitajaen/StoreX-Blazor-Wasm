...
public partial class ProductEntityTable<TEntity, TId, TRequest>
    where TRequest : new()
{
    [Parameter]
    [EditorRequired]
    ...
    private async Task CustomDelete(TEntity entity)
    {
        _ = OrderIdFunc ?? throw new InvalidOperationException("OrderIdFunc can't be null!");
        _ = ProductIdFunc ?? throw new InvalidOperationException("ProductIdFunc can't be null!");
        TId orderId = OrderIdFunc(entity);
        TId productId = ProductIdFunc(entity);

        string deleteContent = L["You're sure you want to delete {0}?"];
        var parameters = new DialogParameters
        {
        ...
        if (!result.Cancelled)
        {
            _ = CustomDeleteFunc ?? throw new InvalidOperationException("CustomDeleteFunc can't be null!");

            await ApiHelper.ExecuteCallGuardedAsync(
                () => CustomDeleteFunc(orderId, productId),
                Snackbar);
    ...
    [Parameter]
    public Func<TEntity, TId>? OrderIdFunc { get; set; }
    [Parameter]
    public Func<TEntity, TId>? ProductIdFunc { get; set; }
    [Parameter]
    public Func<TId, TId, Task>? CustomDeleteFunc { get; set; }

    [Parameter]
    public bool IsMultiSelect { get; set; } = false;
    [Parameter]
    public HashSet<TEntity> SelectedItems { get; set; } = new HashSet<TEntity>();
    [Parameter]
    public EventCallback<HashSet<TEntity>> SelectedItemsChanged { get; set; }
